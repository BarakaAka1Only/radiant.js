# Contributing
Community patches, bug reports, and contributions are always welcome and are crucial to ensure `radiant.js` stays stable. Here are steps/guidelines to follow when contributing.
...
https://github.com/BarakaAka1Only/radiant.js/issues
...
## Submitting changes
To submit your changes, please open a pull request from your branch to the `radiant.js` `master` branch. If this is for an inter-dependent branch, please still point to the `master` branch. When one PR is landed, then the other should automatically land as well.
...
## Approval process
Due to the brittle nature of `radiant.js`, we must be flexible to allow for quick deployment of fixes. Please use the following guidelines when approving/landing changes.
